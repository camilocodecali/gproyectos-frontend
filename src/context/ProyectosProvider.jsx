...
const ProyectosProvider = ({ children }) => {
  ...
  const nuevoProyecto = async (proyecto) => {
    try {
      ...
      //Socket io
      socket.emit("nuevo proyecto", data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const editarProyecto = async (proyecto) => {
    try {
      ...
      //Socket io
      socket.emit("actualizar proyecto", data);
  ...
  const eliminarProyecto = async () => {
    try {
      ...
      socket.emit('eliminar proyecto', data)
  ...
  //Socket io
  const submitProyectoSocket = (proyecto) => {
    setProyectos([...proyectos, proyecto]);
  };

  const actualizarProyectoSocket = (proyecto) => {
    //Sincronizar el state
    const proyectosActualizados = proyectos.map((proyectoState) =>
      proyectoState._id === proyecto._id ? proyecto : proyectoState
    );
    setProyectos(proyectosActualizados);
  };

  const eliminarProyectoSocket = (proyecto) => {
//Sincronizar el state
  ...
  return (
    <ProyectosContext.Provider
      value={{
        ...
        submitProyectoSocket,
        actualizarProyectoSocket,
        eliminarProyectoSocket
      }}
    >
      {children}
    </ProyectosContext.Provider>
